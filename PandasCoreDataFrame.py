...
data_dir = 'E:\\pythonworld\\python_analysis_exercise\\PythonScript\\Learningpythonscript\\'
#filename = '2.txt'
...
#TemProfileLitersPerHour = ['800l/h', '600l/h', '400l/h' '200l/h']
# temp le rahe ... liter/hour , eg : 800L/Hour
...
pointtypes = ['-.', '-.', '-.', '-.'] # line style for plotting
markers = ['+','x', 'o', 'v'] #marker style for plotting
labels = ['800l/h', '600l/h', '400l/h', '200l/h'] #label for diffrent flow rates
out_dir = '/e/pythonworld/python_analysis_exercise/PythonScript/Learningpythonscript' 
out_dirs = [out_dir + x + "l_h" for x in litersPerHour]   # a list of directories for each flow rates 
...
def read_data(fpath):
    if fpath == filepaths[0]:
        df = pd.read_csv(os.path.join(fpath), sep = ";", skiprows = 10, header = None, encoding='utf-8')
    elif fpath == filepaths[1]:
       df = pd.read_csv(os.path.join(fpath), sep = ";", skiprows = 21, header = None, encoding='utf-8')
    elif fpath == filepaths[2]:
       df = pd.read_csv(os.path.join(fpath), sep = ";", skiprows = 25, header = None, encoding='utf-8')
    elif fpath == filepaths[3]:
       df = pd.read_csv(os.path.join(fpath), sep = ";", skiprows = 30, header = None, encoding='utf-8')
    return df
############################################
...
Tem2Code = data2Code(Dfs) # converting temputure into codes
...
################################
##Function to perfrom non-dimensionalisation of time. Note that I have used pd.to_datetime to convert the date time series in
...
##pandas dataframe.
################################
def Tmix_average(dfs): #dfs is expected to be a list of dataframes.
    val = [df.mean(axis = 1) for df in dfs] #df.mean(axis=1) called on each DF. 'mean' caluculate the mean along the specifies axis. axis=1 => mean should be calculated across the columns.(each row)
    return val # new list of series objects, where each series contains the row means for a DF
...
#p(rho) - density of fluid => 1000
#cp => heat capacity of fluid.=> 4.186
#v - volume of fluid=> 0.397
#h - height (level) of fluid in tank
#'memix' function loops over two dataframe in the list and prforms the caluculation for each element x.
#energy moment = 1000*4.186*0.397*x*0.835
###################################
...
#################################
def sum_ME_Exp(dfs): #calculate the row-wise sum of each dataframe in list
    val = [x.sum(axis = 1) for x in dfs] #list comprehension to iterate over each dataframe 'x' in 'dfs'. 
    #for each dataframe, 'x.sum(axis=1)' computes the sum across each row ('axis=1')
    return val
...
def y_B(dfs):
    ...
    for i, df in enumerate(dfs):
        th = 60 #th is for hot temp
        tc = 10 #tc is for cold temp
        for x in df:
...
for i, mix in enumerate (MixNumber):
    print ('inner list in MixNumber= ', i, len(mix), mix )
print (timeScaled)
...
def multiplot(mixnumber, #data to be plotted on y-axis. maye list/array of multiple datasets
               timedata, #data o be plotted on x-axis. 
              outpath, #file path where the plot will be saved. string indicating the locatiion 
              colors,
              point_types, #o,s,^ etc
              labels,
              markers,
              ...
              transparent = False, 
              ylimit = (),
              xlimit = ()): #used for comparing diffrent dataset/visulizing multiple series of data with different properties.
    plt.figure()
    ...
    #plt.legend()
    #plt.savefig(outpath)
    if ylimit != (): #get the limit y asix
        plt.ylim(ylimit)
    if xlimit != ():
        plt.xlim(xlimit)
    for mix_number, time_data, color, label, pointtype, marker in zip(mixnumber, timedata, colors, labels, point_types, markers): #Parallel iteration
        ydata =  mix_number
        xdata = time_data
        ax= plt.subplot(111)
